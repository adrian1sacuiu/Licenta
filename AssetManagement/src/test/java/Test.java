...
public class Test {
	public static void main(String[] args) {
		 ...
		 * ClassPathXmlApplicationContext("spring-config.xml"); UsersService
		 * usersService = (UsersService)ctx.getBean("usersService");
		 * 
		 * try { User user = new User(); user.setUsername("test");
		 * user.setPassword("test"); user.setEmail("test@test.com");
		 * //usersService.addUser(user);
		 * 
		 * user.setIdUser(3l); user.setPassword("test2");
